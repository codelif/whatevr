...
| DN17 | Video messages show up as empty rows; videos still do not play in any backend; "Currently in use" on the Advanced page shows nothing; make the voice bubble pixel perfect, and inline the timestamp for every kind that has room | done | Four reports, two root causes, both of them regressions in the media pass that shipped just before. **(a) Empty rows, and "no video in any backend", are two different bugs that looked like one.** The empty row is a plain visibility mistake: removing the square plate behind a round instant video was done as `visible: false` on ChatBubble's `bubble` rectangle, which is the **parent of `contentColumn`**, so the circle, its ring, its mute toggle and its footer all went invisible with the plate. Fixed by making video notes a proper frameless row instead: `frameless: isSticker \|\| isJumboEmoji \|\| isVideoNote`, `bubble.visible: !frameless`, `hasInlineMedia` narrowed to the kinds that really do drive the bubble width (`isImage \|\| isVideo \|\| isGif`), and one `Loader` for `VideoBubble` inside `FramelessBubble`'s slot, sized from `videoNoteDiameter`. That is the same path stickers and jumbo emoji already take, so the note gets the opaque time/ticks pill under it, and the glow / selection / reaction geometry, for free; text rows never build the subtree, so the delegate budget is untouched. **(b) Dead video and the blank Advanced page were one bug, and it is DN1 again.** `MediaBackend` declared `explicit MediaBackend(QObject *parent = nullptr)`, and `QQmlPrivate::singletonConstructionMode()` tests `std::is_default_constructible` *before* `HasSingletonFactory`, so the engine built a **second** MediaBackend and never called `create()`. `main()` resolved instance A; QML read instance B, which had never resolved: `description` empty (the blank settings row) and `resolved` false, which `VideoSurface.qml` gated its Loader on, so **no decoder was ever built, in either engine**. Proven rather than guessed: with resolution moved into the accessors the log printed the engine line **twice**, one per instance. Fixed by dropping the defaulted `parent`, exactly as DN1 did for `ProtocolController` and `Settings`. Kept from the diagnosis: the accessors resolve on first read (so nothing can act on a placeholder), the choice is announced once with `qInfo` (a report of "video is broken" now carries the engine), the emit is queued (emitting inside the QML binding that triggered resolution is a binding loop), and `VideoSurface` no longer gates on a flag at all. Note for the next person reading a log: Qt routes messages to journald when stderr is not a tty, so a redirected run looks silent; `QT_FORCE_STDERR_LOGGING=1` puts it back. **(c) Inline timestamps, generalised.** `tntFitsInline` was text-only; every other kind paid a full extra row. Split into `tntFitsAfterBody` (unchanged), `tntFitsAfterReadMore` (the "Read more" line is short, so the footer rides its right end) and `tntFitsInAttachment` (voice, audio and documents put it on the bottom line they already draw, with `tntReserveWidth` kept clear for it), all feeding the same `footerSlot`. Voice and audio bubbles lose a row; `attachmentBlockHeight` drops with them. **(d) Voice pixel pass.** Filled circular play button with the download ring on its rim instead of inside it, waveform spanning the row, resume dot on the waveform's centre line rather than hanging below it, bar pitch 2/2 → 3/3 (130 hairlines in a row drew a fence), bar widths snapped to whole device pixels, and the envelope **scaled to its own peak**: senders ship steady speech as 40-80 out of 100, and drawn literally that is a flat picket fence, with a floor on the divisor so a genuinely quiet note stays quiet. Two more bugs found while looking: tapping a video note that had not downloaded did nothing at all (`activate()` only toggled sound), and `MediaDragArea` logged a binding loop on every media row because `Drag.active` was bound to the handler the drag system also writes. Verified: `just build`, `ctest` 7/7 with new `voice` and `video-note` rows in `tst_chatbubbleperf` (128 and 176 objects) and two new waveform cases, Go suite, `just validate`, and **the real client run against the live daemon and screenshotted**: circle over the wallpaper with its ring and pill, no stray chrome, voice bubbles compact with the time inline. |

| DN18 | Video playback is erratic (inline sometimes works, sometimes not), the full-screen viewer is a black screen, "Cannot load libcuda.so.1" in the log, video messages do not work at all, Qt Multimedia dumps ffmpeg's stream banner, and the video that does play is upside down | done | Five reports, and the first real answer came from a source outside the codebase: Harsh asked why we hand-wrote an mpv wrapper instead of using KDE's **MpvQt** (invent.kde.org/libraries/mpvqt, the wrapper Haruna is built on). His call was to keep ours and mine theirs, which was the right one, because reading their `mpvrenderer.cpp` found four places where we had libmpv's contract wrong. **(a) The root cause of "no video in any backend" is an ordering bug, and mpv says so out loud.** With `vo=libmpv`, mpv decides whether a file gets video *when the file is loaded*: no render context at `loadfile` time means `[vo/libmpv] No render context set`, `Error opening/initializing the selected video_out (--vo) device`, `Video: no video`, and the file then plays through to eof with its video track disabled. The context arriving a frame later does not undo that. We loaded on the GUI thread the instant the pool handed over a core, while the context is only built on the render thread's next sync, so we lost that race routinely. MpvQt has the handshake for exactly this (`MpvAbstractItem::ready()`); I had explicitly listed it as "not adopted" in the plan, which was wrong. `MpvVideoItem` now holds the file until `notifyRendererReady()` arrives from the renderer. This was invisible until it was not: the fix came from a `WHATKEVR_MPV_LOG=v` switch added mid-debugging, because mpv's own account of what it was doing was being thrown away (`msg-level=all=error`, `terminal=no`). That switch stays. **(b) Upside down was ours, and provably so.** The reported clip carries no display matrix and no rotate tag (`ffprobe`: 478x850, no side data), so the phone renders it exactly as decoded. MpvQt passes `flip_y = 0` and never calls `setMirrorVertically`; we passed `flip_y = 1` with `setMirrorVertically(false)`, which is one flip too many. Settled by reading a working implementation rather than by flipping a flag to see what happened. **(c) Three more from the same file.** `MPV_RENDER_PARAM_WL_DISPLAY` / `X11_DISPLAY` is how mpv reaches the platform's video acceleration, and we passed neither; with it the log now reads `Using hardware decoding (vaapi)` and `VO: [libmpv] 480x480 vaapi[nv12]`. `MPV_RENDER_PARAM_ADVANCED_CONTROL` was set to 1 while we never called `mpv_render_context_update()`, which is the entire contract that flag opts into; removed. And the lifetime: the `mpv_handle` now lives behind a `shared_ptr` shared with the renderer (MpvQt's `MpvHandleManager`), so `mpv_render_context_free()` on the render thread always precedes the handle going away, the update callback is cleared before the free, `mpv_terminate_destroy` replaces `mpv_destroy`, and the frame callback holds a `QPointer` instead of a raw item. **(d) The black full-screen viewer was the pool.** QML assigns `playing` before `reserved` (declaration order in `MpvVideoSurface.qml`), so `attachCore()` ran while the pool still thought the viewer was an ordinary bubble; with `inlineVideoLimit` at its default 3 and three notes autoplaying behind it, `acquire()` returned nullptr and the viewer got no decoder at all. `setReserved()` now re-homes an item that already holds an inline core, `~MpvVideoItem` clears the pool's reserved pointer (a dangling one orphaned the reserved decoder for the rest of the session), and a refused item joins a FIFO that `release()` serves, so "asked once and stayed a thumbnail forever" becomes a wait. New `tst_mpvpool` pins all four. **(e) Video notes were never fetched at all.** All three in the live DB had an empty `media_local_path` and no `.mp4` on disk: `requestPlayback()` preferred `media.stream` (the default), and streaming never writes a local path, so a one-second 250 KB recording was re-fetched and re-decrypted on every scroll past. Video notes and anything under 4 MB now download. Also `playingInline` was `active && playing`, where `active` means "a decoder is attached", not "there is a frame", so the poster was swapped out for an empty surface the moment a core arrived; it is now gated on a real frame (`surfaceHasFrame`, from mpv's `dwidth` and Qt's `hasVideo`), with a spinner over the thumbnail during the gap. One trap on the way: gating the surface's *visibility* on the same flag deadlocks it, because an invisible `QQuickFramebufferObject` is never rendered and so never creates the render context; the surface is visible from the moment playback is wanted and sits at `z: -1` under the poster instead. **(f) The ffmpeg banner was Qt Multimedia's**, routed through its `qt.multimedia.ffmpeg*` categories, and one added logging rule in `main.cpp` silences it (verified: zero banner lines under `WHATKEVR_VIDEO_BACKEND=qt`). **The `Cannot load libcuda.so.1` line is not fixed and is left alone deliberately.** It comes from the ffnvcodec loader compiled into libmpv, and it escapes mpv's log system entirely: `msg-level=all=no`, `terminal=no` and libavutil's own `av_log_set_level` were all tried and none suppress it. It is the hardware-decoding probe walking its list, not a failure; the driver that answers is the result. UI/UX pass alongside: full-screen viewer gains a real failure state (it was a black rectangle that said nothing), auto-hiding chrome, chrome bars that swallow their own taps instead of closing the viewer, and a seek bar whose `value: pressed ? value : surface.position` self-reference became a proper `Binding`; video notes gained a countdown pill; the duration chip counts down; the voice speed pill reserves its width instead of shifting the row on play; the gallery got keyboard navigation and accessible names. Two over-video buttons (mute, download) were `ToolButton`s with replaced backgrounds and had **never drawn a glyph** (the "stray dark blob on the rim" I mis-diagnosed in DN17 as placement); they are `AbstractButton`s with explicit white `Kirigami.Icon`s now. Verified end to end by running the real client under a headless nested sway (`WLR_BACKENDS=headless`, deep-linked into the chat with `whatevr://chat/...`, screenshotted with `grim`) so none of it touched Harsh's live desktop: all three video notes download, persist, and play upright with ring, countdown and a working mute glyph, under **both** mpv and `WHATKEVR_VIDEO_BACKEND=qt`. `just build`, `ctest` 8/8 with the new `tst_mpvpool`, `just validate`, qmllint 152 with none in a touched file, Go suite green. **Not verified, and it needs Harsh's hands:** tapping a rectangular video to play it, and opening the full-screen viewer, both of which need a pointer the headless seat does not have. The mpv render path they use is the same one the notes proved, and the viewer's reservation is covered by `tst_mpvpool`, but neither has been clicked. |
| DN19 | Only one video should ever play at once; do we still need the in-house mpv wrapper; play only on click for videos and video notes, pause when scrolled off screen; several video notes at once "go weird"; clicking a running video opens it full screen while the inline copy keeps running, and from the start; there should be a full screen button; "so many little inconsistencies, it feels so bad to use"; redesign the media UX to be obvious | done | Harsh is right, and the reason was structural rather than cosmetic: three mechanisms decided what a video bubble showed (autoplay, a bounded decoder pool, and a tap that meant three different things depending on kind and state) and they overlapped. Every complaint was one of those overlaps surfacing. **The white shape in the screenshot is not a broken icon:** it is `process-working-symbolic`, `org.kde.desktop`'s `BusyIndicator`. Three notes autoplayed, `MpvPool` capped inline cores at three, and whichever lost the race sat in `buffering` **forever**, because the bubble had no failure timeout at all (only `MediaViewer` had one). **(a) The in-house wrapper stays, slimmed.** Asked again, and the decisive point against MpvQt is that `MpvAbstractItem` creates its own `mpv_handle` per QML item and cannot hand one instance between items, so adopting it would trade a warm reusable core for a full mpv create/destroy on every play, and `MpvCore` would still be needed for voice notes: two mpv abstractions instead of one. The render layer has been MpvQt-correct since DN18. **(b) Arbitration moved above the engine seam.** New `VideoPlaybackArbiter` (`Whatevr.VideoPlayback`), asked once by `VideoSurface.qml` whichever backend it loads. Expressed inside `MpvPool` the rule did not exist at all on the Qt Multimedia path, where each bubble builds its own `MediaPlayer` and **every clip in the conversation played at once**. Two lanes, because two kinds of clip want opposite things: **Exclusive** for videos and video notes, where a request always wins and the previous holder is revoked (a tap must start what you tapped), and **Animated** for GIFs, bounded by the user's limit and first come first served, Harsh's call that GIFs keep a pool of their own. The arbiter also owns resume positions, written by the surface at the one moment it still has a position: as it lets go of the decoder. `MpvPool` drops the reservation, the waiter FIFO and `MpvVideoItem::reserved` with it, and `inlineVideoLimit` becomes `gifPlayerLimit`. **(c) The never-ending spinner cannot recur.** A decoder now exists only for a surface that holds a grant (`source: grantHeld ? source : ""`), so live decoders are bounded by the arbiter rather than by the pool refusing anyone, and a bubble that has a grant but no picture after six seconds says "Could not play" with Try again and Open externally instead of spinning. **(d) One `phase` per bubble.** The file computed six overlapping `visible:` conditions independently, which is how a note showed a spinner on top of a thumbnail next to a play glyph; there is now one `phase` (needsDownload, downloading, idle, buffering, failed, playing, paused), one thing in the middle of the bubble per phase, and one meaning for a tap on every kind: start what is stopped, stop what is running. `userStarted`/`userPaused`/`userUnmuted` collapse into one `intent`, the double tap is gone, and every other action is a visible button: full screen, mute, download. Playback of every kind stops on leaving the viewport, not just autoplay, and resumes where it stopped. **(e) The handoff goes both ways.** The full-screen button carries the second the inline copy reached (`showVideo` grows a `startAt`, applied as mpv's `--start` at load rather than a seek that races the first frame), and it stops the inline copy by the same mechanism that stops everything else: the viewer takes the exclusive lane, which revokes the bubble. Closing the viewer leaves the bubble at the position the viewer reached. **Two corrections to my own work.** I wrote a comment claiming the original full-screen failure was the strip's `Loader` tearing down the button mid-handler and dropping the emit; I then tested that hypothesis by re-adding the line, and it is **wrong**, the handler runs to completion. Comment removed rather than shipped; the reorder stays because one mechanism deciding what plays is better than two, and the original symptom was most likely my own mis-aimed synthetic clicks. And DN18 shipped a real bug I only found by running the second backend: `QtVideoSurface` reported a frame only in `PlayingState`, so every **paused** clip read as "no picture", fell into the buffering phase, and sat under a spinner that then timed out into a failure it had not had, with `VideoOutput` painting black corners around the circle. Fixed. **Verification** used a nested sway on the **X11 backend under Xvfb** rather than DN18's headless backend, which is what finally allowed clicking: the wlroots headless backend attaches no input devices, so its seat advertises no pointer and synthetic `swaymsg seat - cursor` events reach nothing, while the X11 backend brings a real pointer, keyboard and touch device with it, all still off Harsh's desktop. Clicked, on **both** backends: a note plays on click with sound, upright, ring tracking, countdown, and returns to its poster at the end; pausing holds the frame with mute and full-screen on the rim; the rectangular strip pauses and mutes without the tap falling through to the bubble; the full-screen button opens the viewer at the carried position (`0.068s` through the chain in the log). The video-note delegate came out **lighter** than before the redesign, 168 objects against the 190 budget, because the transport, spinner and failure plate are all behind `Loader`s. `just build`, `ctest` 8/8 with `tst_mpvpool` replaced by `tst_videoarbiter` (7 cases, no skips, no libmpv needed), `just validate`, qmllint with zero warnings in any touched file, Go suite green. **Not verified, and it needs Harsh's hands:** the full-screen button on a *rectangular* video specifically. Its `openFullScreen()` is the same function the video note's rim button exercised end to end, and the strip's other buttons were clicked, but that one target I could not land: scrolling far enough up to reach the only long clip in the chat needs kinetic wheel input the synthetic seat does not reproduce. |

### Phase E: teardown & flagship polish

| id | step | status | notes |
| --- | --- | --- | --- |
| E1 | Delete daemon gRPC server + `proto/`; purge protobuf/grpc from go.mod, CMake, justfile, packaging, README dependency lists | done | **The gRPC stack is gone; whatevrd serves exactly one socket.** Deleted `proto/whatevr.proto` and all of `whatevrd/internal/rpc/` (7 services, `server.go`, `session_bus.go`, the 442 KB generated `pb/`), 14 files in all. `go mod tidy` dropped `google.golang.org/grpc` and `genproto/googleapis/rpc`; **`google.golang.org/protobuf` stays** and always will, it is whatsmeow's waE2E wire format and never was ours. **The packaging flip A1 deferred to teardown landed here**: `protocol.New` gained an `activated net.Listener` parameter (nil = standalone), `packaging/systemd/whatevrd.socket` now listens on `%t/whatevr/whatevrd.sock`, and the protocol server tracks `ownsSocket` so it unlinks the socket only when it created it. One bug found doing it: an adopted listener must also `SetUnlinkOnClose(false)` or shutdown unlinks systemd's own socket, leaving systemd accepting on a path no client can reach; `net.FileListener` already defaults that off, so the old gRPC path was accidentally safe, but it is now explicit and tested rather than inherited. `main.go` lost the `multiChatOpener` fan-out (the notification worker takes the protocol server directly), the second server's lifecycle arm, and the two-socket log line. `app.Paths` collapsed to one socket (`SocketDir`/`SocketPath` = `$XDG_RUNTIME_DIR/whatevr/`, PROTOCOL.md's path); **`LockPath` deliberately did not move**: see the Decision log. Also purged: the `just proto` recipe, the `-X whatevrd/internal/rpc.Version` ldflag, `qt6-grpc` from both CI workflows, and the `proto/**` CI path filters; five stale comments that described a gRPC stack that no longer exists were corrected. New tests: `protocol.TestActivatedListenerIsAdoptedAndSocketSurvivesShutdown` / `…SkipsSocketDirValidation` (systemd creates the parent 0755, which the daemon's own path rejects) and `app.TestResolvePathsSocketAndLockLocations`. `just build`, full Go suite, `ctest` 5/5, `scripts/conformance` and `ldd` (no grpc/protobuf in whatkevr) pass. Hand-verified both socket lifecycles end to end under `systemd-socket-activate` and standalone; see the E-phase notes. |
...
- 2026-08-10: **D5 live verification gap (environment, unchanged since D3c),
  ...
  `invalid_params`, so the shape is right); `subscribe starred` global and
...
- 2026-08-10: D4b implementation readings (no PROTOCOL.md change; **one
  ...
  **Narrowing to flag:** that subscription is `archived:false`, so *archived chats
  ...
  in the frontend (merging is forbidden). Say the word and it becomes a sectioned
...
- 2026-08-09: D3c implementation readings (no PROTOCOL.md change; flag if you
  ...
  `typing` is global and unsolicited, `presence` is per-chat and demand-driven, and PROTOCOL keeps them separate
  for exactly that reason, so the header reads
  ...
  pattern) rather than a QSortFilterProxy pair, with no frontend sorting machinery.
...
- 2026-07-07: B5b implementation readings (PROTOCOL.md unchanged, flag if you
  ...
  participant fallback can't know roles/owner/flags), so they land in phase two, and
