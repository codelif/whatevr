...
| DN7 | Windowing the chat list leaves no way to ask the daemon for **one** chat's row | needs-decision | Raised by DN6, not by Harsh. `selectedChatItem()` reads the selected chat's row out of the two `chats` collection models — it is where the conversation header's name and avatar come from, and where `selectedChatUnreadCount()` gets the number that picks the `messages` anchor. While those models held the whole roster that always worked; windowed, a chat opened from a search hit, a notification, or a `whatevr://chat/<id>` link can be a chat the window has not loaded. PROTOCOL.md has no per-chat view: `contact` and `group` are cards, not chat rows, and `chats` cannot be narrowed to a jid. **Shipped as an interim:** `ensureSelectedChatLoaded()` widens the window a page per `ready` until the row arrives or the view is exhausted (window management, not a frontend cache — worst case it degenerates to today's full-roster load, and only for a chat the user actually opened). In practice the hole is small, because a chat worth opening from a notification or with unread messages sorts near the top of a recency-ordered list. **The clean fix is a `chat` object view (`chat_id` → the same row `chats` emits), which is a PROTOCOL.md addition and needs Harsh's sign-off before anything is written.** |
...
- **DN7 needs a decision before it can be implemented** (2026-08-10). Windowing
  the chat list (DN6) showed there is no way to ask the daemon for a single
  chat's row, which the conversation header and the unread anchor both need
  when a chat is opened from outside the loaded window. The proposal is a
  `chat` object view taking `chat_id` and emitting the same row `chats` emits.
  An interim window-chase ships with DN6 so nothing regresses; PROTOCOL.md is
