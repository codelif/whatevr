...
func TestFormatMediaFallback(t *testing.T) {
	content := FormatMessage(Capabilities{Body: true}, app.Message{MediaMimeType: "image/jpeg"}, app.Chat{Name: "Alice"})
...
func TestFormatEscapesMarkup(t *testing.T) {
	content := FormatMessage(Capabilities{Body: true, BodyMarkup: true}, app.Message{Text: "<hello>"}, app.Chat{Name: "Alice"})
